...
export function buildDailyPrompt(client, { stories, carousel }) {
  const parts = [
    ...
    `נדרשים ${stories} סטוריז: לכל סטורי — טקסט הסטורי (הוק, מסר, CTA) + תיאור קונספט ויזואלי + פרומפט תמונה באנגלית התואם את המותג.`,
    'חשוב: טקסט הסטורי חייב להיות מוטבע בתוך התמונה — פרומפט התמונה חייב לכלול הוראה לרנדר את הטקסט העברי המדויק כחלק מהעיצוב. התמונה היא התוצר המוגמר.',
  ];
  if (carousel) {
